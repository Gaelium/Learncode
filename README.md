...
Import EPUB programming books into VS Code, extract code exercises, and learn interactively with a built-in book reader, exercise sandboxes, and progress tracking.

## Getting Started

### 1. Import a Book

Open the Command Palette (`Cmd+Shift+P`) and run **LearnCode: Import EPUB Book**.

1. **Select your EPUB** -- a file picker opens filtered to `.epub` files
2. **Choose a workspace folder** -- the dialog shows the book's title and defaults to the EPUB's directory. Click "Create Workspace Here" to confirm.
...
|---------|------|---------|-------------|
| `learncode.defaultSandboxLocation` | string | `""` | Default directory for creating exercise workspaces. When set, the folder picker defaults to this location instead of the EPUB's directory. |
...
my-book-workspace/
  .learncode/
    ...
    book/                # Extracted EPUB content (HTML + images)
  ch01-introduction/
...
|---------|-------------|----------------|
| LearnCode: Import EPUB Book | Import a new book and create a workspace | Command Palette |
...
| LearnCode: Open Book Reader | Open the EPUB reader panel | Chapter context menu, Command Palette |
